...
type EnsureNotContainUnderscore<T extends string> = [T] extends
  [(T extends string ? T extends `${any}_${any}` ? never : T : never)] ? T : never
...
export type ExhaustivePattern<Tag extends string, T extends Adt<Tag>, R> = EnsureNotContainUnderscore<T[Tag]> extends
  never ? never : NestedKeys<Tag, T> extends infer NK ? Exclude<T[Tag], NK> extends infer OK ? 
